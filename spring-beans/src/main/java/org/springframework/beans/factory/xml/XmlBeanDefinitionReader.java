...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected EntityResolver getEntityResolver() {
		if (this.entityResolver == null) {
			// Determine default EntityResolver to use. 使用默认的 EntityResolver
			ResourceLoader resourceLoader = getResourceLoader();
			if (resourceLoader != null) {
				// ResourceEntityResolver类是DelegatingEntityResolver的子类
				this.entityResolver = new ResourceEntityResolver(resourceLoader);
			}
			else {
				// DelegatingEntityResolver是EntityResolver接口的实现类
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			// 已经封装好的encodedResource，从Resource中获取内容的InputStream
			InputStream inputStream = encodedResource.getResource().getInputStream();
			try {
				// InputSource这个类来源于jdk org.xml.sax.InputSource
				InputSource inputSource = new InputSource(inputStream); // 构造InputStream
				if (encodedResource.getEncoding() != null) {
					inputSource.setEncoding(encodedResource.getEncoding());
				}
				//通过构造的inputSource和Resource调用doLoadBeanDefinitions函数,doLoadBeanDefinitions是处理配置文件真正逻辑核心部分
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			...
			// 记录加载本次加载的BeanDefinition个数
			int count = registerBeanDefinitions(doc, resource); //注册配置文件
	 ...
	 *  将加载配置文件的任务委托给了 DefaultDocumentLoader.loadDocument()
	 *   getEntityResolver(): 项目本身提供一个如何寻找DTD声明的方法，避免通过网络来寻找相对应的声明
	 *   getValidationModeForResource(resource) 获取配置文件的模式：DTD 还是XSD(xml模式)
	 */
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
			//detectValidationMode将实现自动检测的验证模式的功能委托给了XmlValidationModeDetector.validationModeDetector方法
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 通过DefaultBeanDefinitionDocumentReader 实例化 BeanDefinitionDocumentReader
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		//记录统计前BeanDefinition的加载个数
		int countBefore = getRegistry().getBeanDefinitionCount();
// 加载及注册bean （加载XML文件的核心）
	...
	protected BeanDefinitionDocumentReader createBeanDefinitionDocumentReader() {
		return BeanUtils.instantiateClass(this.documentReaderClass);// BeanUtils.instantiateClass(a.calss)实例化类
	}
